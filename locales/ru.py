...
✅ <i>FPW</i> получает полную историю чата и видит все данные о всех новых сообщениях.
✅ <i>FPW</i> может видеть изображения в чате и пересылать их в <i>Telegram</i> чат.
✅ <i>FPW</i> может точно определить, кто именно написал, будь то ты, твой собеседник или 3е лицо (арбитраж).
❌ Из-за того что <i>FPW</i> получает полную историю чата для обнаружения новых сообщений, чат становится "прочитанным" (не загорается оранжевым).
...
❌ <i>FPW</i> не получает полную историю чата, вследствие чего видит только последнее сообщение, а не все. Если пользователь быстро напишет несколько сообщений, <i>FPW</i> увидит только последнее.
❌ <i>FPW</i> не может видеть изображения в чате и пересылать их в <i>Telegram</i> чат.
❌ <i>FPW</i> не может точно определить, кто именно написал: ты или собеседник. Если чат не прочитан, значит сообщение от собеседника, иначе - от тебя. Однако, если ты смотришь чат в момент получения сообщений, данная логика иногда может сбоить. Так же <i>FPW</i> не сможет определить, что в чат написало 3е лицо (арбитраж).

❗ Если нажать кнопку <code>Больше</code> в уведомлении о новом сообщении, <i>FPW</i> "прочитает" чат и покажет последние 15 сообщений, в том числе изображения. Так же <i>FPW</i> сможет определить, кто именно автор сообщений."""
...
📋 Если <i>FPW</i> долго не инициализируется, проверь логи с помощью команды /logs"""

FPW_init = """✅ <b><u>FunPay Cardinal инициализирован!</u></b>\n
...
<i>Telegram чат:</i> @FunPay_Westl
<i>Разработчик:</i> @xallse"""

sys_info = """<b><u>Сводка данных</u></b>

<b>ЦП:</b>
{}
    Используется <i>FPW</i>: <code>{}%</code>
...
update_done = "✅ Установил обновление! Перезапусти <i>FPW</i> с помощью команды /restart"
update_done_exe = "✅ Установил обновление! Новый <code>FPW.exe</code> находится в папке <code>update</code>. "\
                  "Выключи <i>FPW</i>, перенеси новый <code>FPW.exe</code> на место старого " \
