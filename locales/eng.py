...
✅ <i>FPW</i> gets the full chat history and sees all data about all new messages.
✅ <i>FPW</i> can see images in chat and forward them to <i>Telegram</i> chat.
✅ <i>FPW</i> can determine exactly who wrote, whether it was you, your interlocutor, or a 3rd party (arbitrator).
❌ Because <i>FPW</i> gets the full chat history to detect new messages, the chat becomes "read" (not lit orange).
...
❌ <i>FPW</i> doesn't get the full chat history, so it sees only the last message. If the user quickly writes several messages, <i>FPW</i> will see only the last one.
❌ <i>FPW</i> cannot see images in chat and forward them to <i>Telegram</i> chat.
❌ <i>FPW</i> cannot determine exactly who wrote: you or the person you are chatting with. If the chat is not read, then the message is from the interlocutor, otherwise it is from you. However, if you are viewing the chat when you receive messages, this logic can sometimes fail. Also, <i>FPW</i> will not be able to determine if a 3rd party (arbitrator) wrote into the chat.

❗ If you click the <code>More</code> button in a new message notification, <i>FPW</i> will "read" the chat and show the last 15 messages, including images. <i>FPW</i> will also be able to determine who the author of the messages is."""
...
📋 If <i>FPW</i> does not initialize for a long time, check the logs with /logs"""

FPW_init = """✅ <b><u>FunPay Cardinal initialized!</u></b>\n
...
<i>Telegram chat:</i> @FunPay_West
<i>Developer:</i> @xallse"""

sys_info = """<b><u>Data summary</u></b>

<b>CPU:</b>
{}
    Used by <i>FPW</i>: <code>{}%</code>

<b>RAM:</b>
    Total:  <code>{} MB</code>
    Used:  <code>{} MB</code>
    Free:  <code>{} MB</code>
    Used by <i>FPW</i>:  <code>{} MB</code>
...
update_done = "✅ The update is installed! Restart the FPW with the /restart command."
update_done_exe = "✅ The update is installed! New <code>FPW.exe</code> is in <code>update</code> folder. "\
                  "Turn off <i>FPW</i>, replace old <code>FPW.exe</code> with new one and run <code>Start.bat</code>. "
...
desc_gs = "Here you can turn the basic <i>FPW</i> functions on and off."
...
cmd_restart = "restart FPW"
cmd_power_off = "shutdown FPW"
